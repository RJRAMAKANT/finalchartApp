...
<body>
	...
	%>
		<% ArrayList<GraphByCenterYear> myList = (ArrayList<GraphByCenterYear>) session.getAttribute("newdata"); %>
	<div class="firstform">
				...
				<div id="secondstep" class="secondstep1">
					<div id="selectDiv" style="display: ; margin-top: 10px;">
						<label for="cars">CHOOSE A CENTER:</label> <select id="cars"
							name="center">
							<option value="<%=request.getAttribute("data2")%>"><%=request.getAttribute("data2")%></option>
							<%
...
window.onload = function () {
	...
	var chart = new CanvasJS.Chart("chartContainer", {
		...
		title:{
			text: "STROKE CASES BY CENTER (<%=request.getAttribute("data2")%>) IN  YEARS "
		},
		axisY: {
			title: "TOTAL CASES"
		},
		data: [{        
			...
			dataPoints: [    
			         	<% for(int i=0;i<myList.size();i++){
							%>
							<%
							GraphByCenterYear alltable=(GraphByCenterYear)myList.get(i);
							
							%>
							{ y: <%=alltable.getYearByData() %>, label: "<%=alltable.getYear() %>" },
				               <% 
							}%>
			<%--{ y: 1000, label: "2018" },
				{ y: 1900,  label: "2019" },
